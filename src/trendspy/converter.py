...
class TrendsDataConverter:
    ...
    def token_to_bullets(token_data):
        ...
        bullets = [
            item.get("complexKeywordsRestriction", {})
            .get("keyword", [""])[0]
            .get("value", "")
            for item in items
        ]
        metadata = [
            next(iter(item.get("geo", {"": "unk"}).values()), "unk") for item in items
        ]
    ...
    def interest_over_time(widget_data, keywords, time_as_index=True):
        ...
        df_data = np.array(extract_column(timeline_data, "value")).reshape(
            len(timeline_data), -1
        )
        ...
        timestamps = extract_column(
            timeline_data, "time", f=lambda x: int(x) if x else None
        )
        ...
        if time_as_index:
            return pd.DataFrame(
                df_data, index=pd.DatetimeIndex(timestamps, name="time [UTC]")
            )
    ...
    def related_queries(widget_data):
        ...
        result["top"] = pd.DataFrame(
            flatten_data(
                filter_data(
                    ranked_data[0]["rankedKeyword"], _RELATED_QUERIES_DESIRED_COLUMNS
                ),
                ["topic"],
            )
        )
        result["rising"] = pd.DataFrame(
            flatten_data(
                filter_data(
                    ranked_data[1]["rankedKeyword"], _RELATED_QUERIES_DESIRED_COLUMNS
                ),
                ["topic"],
            )
        )
    ...
    def geo_data(widget_data, bullets=None):
        ...
        found_cols = set(filtered_data[0].keys()) & {
            "coordinates",
            "geoCode",
            "geoName",
            "value",
        ...
        if "coordinates" in found_cols:
            df_data["lat"] = extract_column(
                filtered_data, "coordinates", f=lambda x: x["lat"]
            )
            df_data["lng"] = extract_column(
                filtered_data, "coordinates", f=lambda x: x["lng"]
            )

        values = np.array(extract_column(filtered_data, "value")).reshape(
            len(filtered_data), -1
        )
    ...
    def rss_items(data):
        ...
        items = list(
            map(lambda item: parse_xml_to_dict(item, "ht:"), item_pattern.findall(data))
        )
        return items
    ...
    def trending_now_showcase_timeline(data, request_timestamp=None):
        ...
        timestamps = [
            timestamp // time_offset * time_offset - time_offset * i
        ...
        for item in data:
            res[item[0]] = np.pad(
                item[1], (0, max_len - len(item[1])), mode="constant", constant_values=0
            )
