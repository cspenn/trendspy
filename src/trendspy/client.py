...
HOT_TRENDS_URL = "https://trends.google.com/trends/hottrends/visualize/internal/data"

# ----------- API LINKS -------------
API_URL = "https://trends.google.com/trends/api"
...
API_TOKEN_URL = "https://trends.google.com/trends/api/widgetdata"
...
# --------------- RSS -----------------
DAILY_RSS = "https://trends.google.com/trends/trendingsearches/daily/rss"
REALTIME_RSS = "https://trends.google.com/trending/rss"
...
class Trends:
    ...
    def __init__(
        ...
        use_enitity_names=False,
        proxy=None,
        session_file=".trendspy_session.pkl",
        persist_cookies=True,
        use_tor=False,
        use_tls_impersonation=True,
        tls_browser="chrome131",
        **kwargs,
    ):
        """
        ...
        if isinstance(language, str) and len(language) >= 2:
            ...
            "hl" in kwargs and isinstance(kwargs["hl"], str) and len(kwargs["hl"]) >= 2
        ):
    ...
    def _extract_keywords_from_token(self, token):
        if self.use_enitity_names:
            ...
            return [
                ...
                for item in token["request"]["comparisonItem"]
            ]
    ...
    def _parse_protected_json(response: requests.models.Response):
        ...
        if (response.status_code != 200) or (content_type not in valid_content_types):
            raise ValueError(
                f"Invalid response: status {response.status_code}, content type '{content_type}'"
            )
    ...
    def _encode_items(self, keywords, timeframe="today 12-m", geo=""):
        ...
        if not all(max_len % length == 0 for length in lengths):
            raise ValueError(
                f"Ambiguous input sizes: unable to determine how to combine inputs of lengths {lengths}"
            )
    ...
    def _get_token_data(
        self, url, params=None, request_fix=None, headers=None, raise_quota_error=False
    ):
        """
        ...
        if raise_quota_error:
            user_type = (
                token.get("request", {}).get("userConfig", {}).get("userType", "")
            )
    ...
    def interest_over_time(
        ...
        token, data = self._get_token_data(
            EMBED_TIMESERIES_URL, locals(), headers=headers
        )
        if return_raw:
            return token, data
        ...
        if token["type"] == "fe_multi_range_chart":
            ...
            return TrendsDataConverter.multirange_interest_over_time(
    ...
    def related_queries(
        ...
        token, data = self._get_token_data(
            EMBED_QUERIES_URL, locals(), headers=headers, raise_quota_error=True
        )
        if return_raw:
            return token, data
    ...
    def related_topics(
        ...
        headers = headers or {"referer": "https://trends.google.com/trends/explore"}
        token, data = self._get_token_data(
            EMBED_TOPICS_URL, locals(), headers=headers, raise_quota_error=True
        )
        if return_raw:
            return token, data
        return TrendsDataConverter.related_queries(data)
    ...
    def interest_by_region(
        ...
        token, data = self._get_token_data(
            EMBED_GEO_URL, locals(), request_fix=data_injection
        )
        if return_raw:
            return token, data
    ...
    def trending_stories(
        self, geo="US", category="all", max_stories=200, return_raw=False
    ):
        """
        Old API
    ...
    def daily_trends_deprecated(self, geo="US", return_raw=False):
        ...
        data = [
            TrendKeywordLite.from_api(item)
            for day in data
    ...
    def trending_now(
        self, geo="US", language="en", hours=24, num_news=0, return_raw=False
    ):
        """
    ...
    def trending_now_showcase_timeline(
        self, keywords, geo="US", timeframe=BatchPeriod.Past24H, return_raw=False
    ):
        req_data = [
            None,
            None,
            [[geo, keyword, timeframe.value, 0, 3] for keyword in keywords],
        ]
        ...
        data = TrendsDataConverter.trending_now_showcase_timeline(
    ...
    def categories(self, find: str = None, language: str = None) -> List[dict]:
        ...
        if cur_language not in self._category_cache:
            ...
            data = self._parse_protected_json(req)
            self._category_cache[cur_language] = create_hierarchical_index(
                data, join_ids=False
            )
    ...
    def geo(self, find: str = None, language: str = None) -> List[dict]:
        ...
        if cur_language not in self._geo_cache:
            ...
            data = self._get(API_GEO_DATA_URL, {"hl": cur_language, "tz": self.tzs})
