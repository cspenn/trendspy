...
def _decode_trend_datetime(date_str):
    ...
    return (
        datetime.strptime(date_str, DATE_T_FORMAT)
        if "T" in date_str
        else datetime.strptime(date_str, DATE_FORMAT)
    )
...
def _process_two_dates(date_part_1, date_part_2):
    ...
    if ("T" in date_part_1 or "T" in date_part_2) and abs((date_1 - date_2).days) > 7:
        raise ValueError(
            f"Date difference cannot exceed 7 days for format with hours: {date_part_1} {date_part_2}"
        )
...
def _process_date_with_offset(date_part_1, offset_part):
    ...
    if "T" in date_part_1 and (
        (unit == "d" and count > 7) or (unit == "H" and count > 7 * 24)
    ):
        raise ValueError(
            f'Offset cannot exceed 7 days for format with time: {date_part_1} {offset_part}. Use YYYY-MM-DD format or "today".'
        )
...
def convert_timeframe(timeframe, convert_fixed_timeframes_to_dates=False):
    ...
    timeframe = timeframe.replace("now", utc_now.strftime(DATE_T_FORMAT)).replace(
        "today", utc_now.strftime(DATE_FORMAT)
    )
    ...
    if len(parts) != 2:
        raise ValueError(
            f"Invalid timeframe format: {timeframe}. Expected format: '<date> <offset>' or '<date> <date>'."
        )
...
def verify_consistent_timeframes(timeframes):
    ...
    if all(td == timedeltas[0] for td in timedeltas):
        ...
        raise ValueError(
            f"Inconsistent timeframes detected: {[str(td) for td in timedeltas]}"
        )
...
def check_timeframe_resolution(timeframes):
    ...
    if len(set(resolution_values)) > 1:
        ...
        for timeframe, delta, (resolution, time_range) in zip(
            timeframes, deltas, resolutions
        ):
            error_message += (
