...
def flatten_tree(node, parent_id="", result=None, join_ids=True):
    ...
    result.append({"name": node["name"], "id": full_id})
...
class HierarchicalIndex:
    ...
    def id_search(self, id_query: str) -> List[dict]:
        ...
        if self.partial_id_search:
            ...
            return [
                item for item in self.name_to_item.values() if id_query in item["id"]
            ]
        else:
            # For categories - only exact matches
            return [
                item for item in self.name_to_item.values() if item["id"] == id_query
            ]


def create_hierarchical_index(
    tree_data: dict, join_ids: bool = True
) -> HierarchicalIndex:
    """
